...
{
    public class FunzaQuoteDBRepository : AbstractDBRepository, IFunzaQuoteReferenceDBRepository
    {
        public FunzaQuoteDBRepository(IAmbientDbContextLocator ambientDbContextLocator) : base(ambientDbContextLocator)
        {
        }

        public OperationResponse<IEnumerable<QuoteReference>> GetAll()
        {
            var result = new OperationResponse<IEnumerable<DomainModel.Funza.QuoteReference>>();
            try
            {
